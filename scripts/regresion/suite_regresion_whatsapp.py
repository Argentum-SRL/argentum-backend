...
def resolver_datos_base(db: Session):
    emails = ["usuario5@argentum.test", "usuario2@argentum.test", "usuario4@argentum.test"]
    usuarios = {}
    for email in emails:
...
def p3_caso_1(datos):
    u = datos["usuario5@argentum.test"]["usuario"]
...
def p3_caso_8(datos):
    u = datos["usuario2@argentum.test"]["usuario"]
    def test(conn, Session, respuestas):
        db = Session()
        conn.execute(text("UPDATE conversaciones_wpp SET slot_filling_activo = false, accion_ejecutada = 'test' WHERE usuario_id = :uid"), {"uid": u.id})
...
def p3_caso_10(datos):
    u = datos["usuario4@argentum.test"]["usuario"]
    def test(conn, Session, respuestas):
        conn.execute(text("UPDATE conversaciones_wpp SET slot_filling_activo = false, accion_ejecutada = 'test' WHERE usuario_id = :uid"), {"uid": u.id})
        respuestas.clear()
...
def p4_variante_no(datos, msg_variante, email="usuario2@argentum.test"):
...
def p5_caso_4(datos):
    u = datos["usuario2@argentum.test"]["usuario"]
    b = datos["usuario2@argentum.test"]["billeteras"]["Mercado Pago"]
    def test(conn, Session, respuestas):
        db = Session()
        conn.execute(text("UPDATE conversaciones_wpp SET slot_filling_activo = false, accion_ejecutada = 'test' WHERE usuario_id = :uid"), {"uid": u.id})
...
def p6_ejecutar_caso(datos, nombre_caso, ent):
    ...
    def test(conn, Session, respuestas):
        ...
        if ent.get("moneda") == "USD" and "Efectivo USD" in (ent.get("billetera_origen") or ""):
            email = "usuario5@argentum.test"
        elif "Galicia" in (ent.get("billetera_destino") or ""):
            email = "usuario5@argentum.test"
        else:
            email = "usuario2@argentum.test"
...
def correr_suite_completa():
    ...
    escenarios = [
        ...
        },
        {
            ...
            "ejecutar": lambda: p4_variante_no(datos, "no fue en galicia", email="usuario5@argentum.test"),
            "esperado": "Voy a anotar $5.000 en Kiosco desde Galicia. ¿Va?",
            "match": "exacto",
        },
        {
